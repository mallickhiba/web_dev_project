...
import { fetchPhotographys, addToFavorites, bookVenue } from '../redux/serviceActions.js';
...
const Photographys = () => {
  ...
  const { photographys, loading, error, favorites } = useSelector((state) => state.photographys);
  ...
  useEffect(() => {
    dispatch(fetchPhotographys({ page, limit, search, sort, filters: appliedFilters }));
          ...
          <Grid container spacing={4}>
            ...
            <Grid item xs={9}>
              <div className="row g-4">
                ...
                {loading && <p>Loading...</p>}
                {error && <p>{error}</p>}
                {Array.isArray(photographys) && photographys.map((service) => (
  <ServiceCard
    key={service._id}
    service={service}
